...
class FollowerRepository {
    private val datasetUserFollower = MutableLiveData<GenericResponse<UserResponse>>()
    val isLoading = MutableLiveData<Boolean>()

    fun callApiUserFollower(username : String){
        isLoading.value = true
        ApiConfig.instance.listFollowing(username).enqueue(object :
            Callback<GenericResponse<UserResponse>> {
            override fun onResponse(
                call: Call<GenericResponse<UserResponse>>,
                response: Response<GenericResponse<UserResponse>>
            ) {
                if (response.isSuccessful) {
                    datasetUserFollower.value = response.body()
    ...
    fun observerUserFollower(): LiveData<GenericResponse<UserResponse>> {
        return datasetUserFollower
    }
}